...
class MetaWorldSawyerEnv(gym.Env):
    def __init__(self, env_name, seed=False, randomize_hand=True, sparse: bool = False, horizon: int = 250):
        ...
        self.recorded_initial_state = None
        self.use_previou_initial_state = True
    ...
    def step(self, action):
        ...
        if self.sparse:
            ...
            reward = reward / 10
        obs_ = trim_mw_obs(obs.astype(np.float32))
        expanded_obs = np.expand_dims(obs_, axis=0)
    ...
    def reset(self, **kwargs):
        ...
        if self.recorded_initial_state is None:
            self.recorded_initial_state = self.get_state()
        else:
            if self.use_previou_initial_state:
                self.set_state(self.recorded_initial_state)
        # Get the obs once to reset history.
        self._get_obs()

        info = {}
        info['success'] = False
        obs_ = self._get_obs().astype(np.float32)
        expanded_obs = np.expand_dims(obs_, axis=0)
        return expanded_obs, info

    def render_rgb(self, mode="rgb_array", camera_name="corner2", width=640, height=480):
